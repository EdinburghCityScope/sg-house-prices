...
const csvFilename = 'house-prices.csv';
...
function interrogateSPARQL(zones) {
    ...
    var bar = new ProgressBar('  API calls :bar :percent :etas', {
        ...
        incomplete: '─',
        width: 55,
        total: Math.ceil(zones.length * 2 / chunk_size)
    });
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?measure ?value
            WHERE {
                ?s qb:dataSet data:house-sales-prices ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   ldim:refArea ?z ;
                   ldim:refPeriod ?y .
